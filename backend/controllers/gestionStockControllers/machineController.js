...
const getMachinesByFactorySchema = Joi.object({
...
const getMachineByFactoryAndNameSchema = Joi.object({
  factoryId: Joi.string()
    .pattern(/^[0-9a-fA-F]{24}$/)
    .required(),
  name: Joi.string().min(1).max(100).required(),
...
exports.createMachine = async (req, res) => {
  try {
    ...
    try {
      ...
      if (factory) {
        await Factory.findByIdAndUpdate(factory, { $addToSet: { machines: machine._id } }, { session })
      }
...
exports.getMachinesByFactory = async (req, res) => {
  try {
    // Validate parameters
    const { error, value } = getMachinesByFactorySchema.validate({
    ...
    const { factoryId, status, search, page, limit } = value
    ...
    console.error("Error fetching machines by factory:", {
      error,
      factoryId: req.params.factoryId,
      query: req.query,
    })
    res.status(500).json({ message: "Server error while fetching machines by factory.", error: error.message })
  }
}
  ...
  try {
    ...
    try {
      ...
      if (existingMachine.factory && existingMachine.factory.toString() !== factory) {
        await Factory.findByIdAndUpdate(existingMachine.factory, { $pull: { machines: req.params.id } }, { session })
      }

      // Add to new factory if factory is provided
      if (factory && existingMachine.factory?.toString() !== factory) {
        await Factory.findByIdAndUpdate(factory, { $addToSet: { machines: req.params.id } }, { session })
      }
...
exports.deleteMachine = async (req, res) => {
  try {
    ...
    try {
      ...
      if (machine.factory) {
        await Factory.findByIdAndUpdate(machine.factory, { $pull: { machines: req.params.id } }, { session })
      }
